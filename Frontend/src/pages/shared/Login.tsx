    ...
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#0f1117',fontFamily:'Inter,sans-serif' }}>
      <div style={{ width:400,background:'#1e2235',border:'1px solid #2a2f45',borderRadius:20,padding:40 }}>
        ...
        <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
          onKeyDown={e=>e.key==='Enter'&&handleLogin()} placeholder="admin@vbworld.in" autoComplete="off"
        ...
        <input type="password" value={password} onChange={e=>setPassword(e.target.value)}
          onKeyDown={e=>e.key==='Enter'&&handleLogin()} placeholder="Enter password" autoComplete="new-password"
        ...
        <button onClick={handleLogin} disabled={loading}
          style={{ width:'100%',padding:13,borderRadius:12,border:'none',background:'#6366f1',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',opacity:loading?.7:1,marginBottom:12 }}>
          {loading?'Signing in...':'Sign In'}
        </button>

        <p style={{ color:'#64748b',fontSize:13,textAlign:'center',margin:'0 0 12px' }}>
        ...
        <div style={{ borderTop:'1px solid #2a2f45',paddingTop:14 }}>
          <p style={{ color:'#64748b',fontSize:11,margin:'0 0 8px' }}>Demo credentials:</p>
